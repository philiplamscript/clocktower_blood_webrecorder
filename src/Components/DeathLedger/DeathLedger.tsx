        ...
        <tbody className="divide-y divide-slate-100">
          {deaths.map((d: Death) => (
            <tr key={d.id} className="h-12">
              ...
              <td className="p-2 border-r border-slate-100"><ClockPicker playerCount={playerCount} label="DEAD" value={d.playerNo} deadPlayers={deadPlayers} onChange={(val) => setDeaths(deaths.map((it: any) => it.id === d.id ? { ...it, playerNo: val } : it))} /></td>
